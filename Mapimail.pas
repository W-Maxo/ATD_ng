...
begin
 ...
 begin
   ...
   if TargetAddr = '' then
      ...
      if TargetName = '' then
         mrdTarget.lpszName := PAnsiChar(AnsiString(TargetAddr))
      else
         mrdTarget.lpszName := PAnsiChar(AnsiString(TargetName));
      mrdTarget.ulRecipClass := MAPI_TO;
      mrdTarget.lpszAddress := PAnsiChar(AnsiString(TargetAddr));
   ...
   if SenderAddr = '' then
      ...
      if SenderName = '' then
         mrdSender.lpszName := PAnsiChar(AnsiString(SenderAddr))
      else
         mrdSender.lpszName := PAnsiChar(AnsiString(SenderName));
      mrdSender.ulRecipClass := MAPI_ORIG;
      mrdSender.lpszAddress := PAnsiChar(AnsiString('SMTP:' + SenderAddr));
   ...
   if MsgSubject = '' then
      ...
      lpszSubject := PAnsiChar(AnsiString(MsgSubject));
   ...
   if ( MsgContent = '' ) AND ( Attachment = '' ) then
      ...
      if MsgContent = '' then
         ...
         lpszNoteText := PAnsiChar(AnsiString(MsgContent));
      ...
      if Attachment = '' then begin
         ...
         mfdAttach.lpszPathName := PAnsiChar(AnsiString(Attachment));
         nFileCount := 1;
         lpFiles := @mfdAttach;
      end;
   end; // with
